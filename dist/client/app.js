...
const toastElement = document.querySelector('#toast-region');
if (!appElement || !toastElement)
    throw new Error('Contêiner da aplicação não encontrado.');
const app = appElement;
const toastRegion = toastElement;
let adminCsrfToken = '';
...
function clientAvatar(code, large = false) {
...
async function api(url, init) {
    ...
    if (!response.ok) {
        ...
        if (response.status === 401 && !url.startsWith('/api/public/') && !url.startsWith('/api/auth/')) {
            location.href = '/login';
        }
        const error = new Error(errorBody.error || 'Erro na requisição.');
...
async function loadAdminSession() {
    try {
        ...
        if (!state.authenticated) {
            location.href = state.needsSetup ? '/setup-admin' : '/login';
            return false;
        }
...
async function downloadXlsx(url, payload) {
    const response = await fetch(url, {
...
function applyTheme(theme) {
    ...
    document.querySelectorAll('[data-theme-icon]').forEach((el) => { el.innerHTML = theme === 'dark' ? icon('sun', 18) : icon('moon', 18); });
    document.querySelectorAll('[data-theme-label]').forEach((el) => { el.textContent = theme === 'dark' ? 'Modo claro' : 'Modo escuro'; });
}
...
function notificationEventIcon(item) {
    if (item.type === 'REPORT_RESPONDED')
        return icon('rocket', 18);
    if (item.type === 'REPORT_LATE')
        return icon('running', 18);
...
function renderNotifications(payload) {
    ...
    const markAll = document.querySelector('#notifications-read-all');
    ...
    if (markAll)
        markAll.hidden = payload.unreadCount === 0;
    if (!list)
        return;
    ...
    list.querySelectorAll('[data-notification-id]').forEach((item) => {
        item.addEventListener('click', async (event) => {
            ...
            try {
                await api(`/api/notifications/${item.dataset.notificationId}/read`, { method: 'POST', body: '{}' });
            }
            catch { }
            location.href = href;
        });
...
async function refreshNotifications() {
    ...
    if (!list)
        return;
    try {
        const payload = await api('/api/notifications?limit=30');
...
function wireNotifications() {
    const trigger = document.querySelector('#notifications-button');
    ...
    markAll?.addEventListener('click', async () => {
        ...
        try {
            ...
            toast(error instanceof Error ? error.message : 'Não foi possível atualizar as notificações.', 'error');
        }
        finally {
...
function loadingAdmin(active = 'dashboard') {
...
let chartSequence = 0;
function emptyState(title, text, action = '', kind = '') {
...
function responseDonut(reports) {
...
function aggregateReportMetrics(reports) {
...
function trendLineChart(reports) {
    const data = aggregateReportMetrics(reports);
    if (!data.length)
        return emptyState('Histórico ainda vazio', 'Quando você criar relatórios semanais, a evolução do CPL aparecerá aqui.');
    ...
    const xFor = (index) => periods.length === 1 ? 120 : 28 + (index / (periods.length - 1)) * 184;
    const yFor = (value) => 45 - ((value - yMin) / Math.max(yMax - yMin, 1)) * 34;
...
function contractsChart(reports) {
    const data = aggregateReportMetrics(reports);
    const contractsData = data.filter((item) => item.contracts > 0);
    if (!contractsData.length)
        return emptyState('Sem contratos informados', 'Quando contratos forem registrados nos feedbacks, a evolução por tese aparecerá aqui.');
    ...
    const xFor = (index) => periods.length === 1 ? 102 : 26 + (index / (periods.length - 1)) * 164;
...
function contractBreakdown(reports) {
    const map = new Map();
    for (const report of reports)
        for (const feedback of report.feedbacks)
            map.set(feedback.thesisName, (map.get(feedback.thesisName) ?? 0) + (feedback.contractCount ?? 0));
    const items = Array.from(map.entries()).filter(([, count]) => count > 0);
    if (!items.length)
        return '<small class="metric-context">Ainda sem contratos por tese</small>';
...
function wireChartTooltips() {
    document.querySelectorAll('[data-chart-point]').forEach((point) => {
        const show = () => {
            ...
            if (!chartId)
                return;
            const tooltip = document.querySelector(`#tooltip-${chartId}`);
            if (!tooltip)
                return;
            try {
                const payload = JSON.parse(point.dataset.tooltip ?? '{}');
        ...
        const hide = () => { const chartId = point.dataset.chartId; if (chartId)
            document.querySelector(`#tooltip-${chartId}`)?.classList.remove('visible'); };
...
function qualityChart(data) {
    ...
    if (!rows.length)
        return emptyState('Sem avaliações ainda', 'As avaliações de qualidade dos leads aparecerão aqui após os primeiros feedbacks.');
...
function problemsInsights(data) {
    if (!data.problemDistribution.length)
        return emptyState('Nenhum padrão registrado', 'Problemas recorrentes serão agrupados aqui quando houver respostas.');
...
function reportRows(reports, showClient = true) {
    if (!reports.length)
        return `<tr class="empty-table-row"><td colspan="${showClient ? 7 : 6}">${emptyState('Nenhum relatório por aqui', 'Crie um relatório semanal para começar a estruturar o histórico.')}</td></tr>`;
...
function wireCopyButtons() {
    document.querySelectorAll('.copy-report-link').forEach((button) => {
...
function setupSteps(hasClients, hasReports) {
    ...
    try {
      ...
      <form id="new-client-form" class="card form-card premium-form">
        ...
        form?.addEventListener('submit', async (event) => {
            ...
            const fd = new FormData(form);
            const payload = { name: String(fd.get('name') ?? ''), thesisSlugs: fd.getAll('thesis').map(String) };
...
function thesisAnalytics(reports) {
    const map = new Map();
    for (const report of reports) {
        for (const metric of report.metrics) {
            const item = map.get(metric.thesisSlug) ?? { slug: metric.thesisSlug, name: metric.thesisName, reports: new Set(), leads: 0, investmentCents: 0, feedbackLeads: 0, feedbackInvestmentCents: 0, contracts: 0, cpls: [] };
...
function thesisPerformanceCards(reports) {
    const rows = thesisAnalytics(reports);
    if (!rows.length)
        return emptyState('Sem dados por tese', 'Crie relatórios para visualizar o desempenho separado de cada tese.');
...
function clientQualityChart(reports) {
    const counts = ['POOR', 'REGULAR', 'GOOD', 'VERY_GOOD'].map((quality) => ({ quality, count: reports.flatMap((report) => report.feedbacks).filter((feedback) => feedback.quality === quality).length }));
    const fakeDashboard = { qualityDistribution: counts };
    return qualityChart(fakeDashboard);
}
async function renderClient(code) {
    loadingAdmin('clients');
    try {
        const data = await api(`/api/clients/${encodeURIComponent(code)}`);
      ...
      <div class="client-content-grid">
        ...
        wireCopyButtons();
        wireChartTooltips();
        document.querySelector('#export-client')?.addEventListener('click', async (event) => {
            const button = event.currentTarget;
            button.disabled = true;
            try {
                await downloadXlsx('/api/exports/general', { clientCode: data.client.code });
                toast('Histórico do cliente exportado.', 'success');
            }
            catch (error) {
                toast(error instanceof Error ? error.message : 'Não foi possível exportar.', 'error');
            }
            finally {
                button.disabled = false;
            }
...
function reportsComparison(reports) {
    const responded = reports.filter((report) => report.status === 'RESPONDED');
    if (responded.length < 2)
        return emptyState('Comparação disponível após dois períodos', 'Com pelo menos dois relatórios respondidos, esta área mostrará a mudança entre as semanas.');
    const current = responded[0];
    const previous = responded[1];
    const sum = (report, key) => report.metrics.reduce((acc, metric) => acc + metric[key], 0);
    ...
    const delta = (a, b) => b === 0 ? '—' : `${a >= b ? '+' : ''}${Math.round((a - b) / b * 100)}%`;
...
function thesisOverviewChart(reports) {
    const rows = thesisAnalytics(reports);
    if (!rows.length)
        return emptyState('Ainda não há teses com histórico', 'As comparações aparecerão quando existirem relatórios com métricas.');
...
async function renderTheses() {
    ...
    try {
      ...
      <article class="card thesis-detail-panel"><div class="table-head"><div><span class="eyebrow">DETALHAMENTO</span><h2>Métricas consolidadas</h2><p>CPL é calculado com investimento ÷ leads. Conversão usa apenas contratos efetivamente informados.</p></div></div>${data.reports.length ? thesisPerformanceCards(data.reports) : emptyState('Nenhum dado para analisar', 'Cadastre um cliente e crie relatórios semanais para liberar a análise de teses.', `<a class="btn btn-primary" href="/relatorios/novo">${icon('plus', 17)} Criar relatório</a>`, 'large')}</article>
        ...
        wireGlobalControls();
        wireChartTooltips();
        const form = document.querySelector('#theses-filters');
        form?.addEventListener('change', () => form.requestSubmit());
        form?.addEventListener('submit', (event) => {
            event.preventDefault();
            const fd = new FormData(form);
...
async function renderReports() {
    ...
    try {
        ...
        const [data, exportData] = await Promise.all([api(`/api/dashboard?${params.toString()}`), api('/api/exports')]);
      ...
      <article class="card table-card export-history-card"><div class="table-head"><div><span class="eyebrow">RASTREABILIDADE</span><h2>Exportações recentes</h2><p>Registro das planilhas geradas pela equipe interna.</p></div></div>${exportData.exports.length ? `<div class="table-scroll"><table><thead><tr><th>Arquivo</th><th>Escopo</th><th>Cliente</th><th>Registros</th><th>Responsável</th><th>Data</th></tr></thead><tbody>${exportData.exports.map((item) => `<tr><td><div class="table-name">${e(item.filename)}</div></td><td>${item.scope === 'REPORT' ? 'Relatório' : 'Geral'}</td><td>${item.clientCode ? `<a class="client-link" href="/clientes/${e(item.clientCode)}">${e(item.clientCode)}</a>` : 'Todos'}</td><td>${item.recordCount}</td><td>${e(item.createdBy)}</td><td>${e(dateTime(item.createdAt))}</td></tr>`).join('')}</tbody></table></div>` : emptyState('Nenhuma exportação ainda', 'As planilhas geradas aparecerão aqui para fins de rastreabilidade.')}</article>
        ...
        wireGlobalControls();
        wireCopyButtons();
        document.querySelector('#export-general')?.addEventListener('click', async (event) => {
            const button = event.currentTarget;
            button.disabled = true;
            try {
                await downloadXlsx('/api/exports/general', { clientCode: data.filters.clientCode, status: data.filters.status });
                toast('Planilha gerada com sucesso.', 'success');
                window.setTimeout(() => void renderReports(), 450);
            }
            catch (error) {
                toast(error instanceof Error ? error.message : 'Não foi possível exportar.', 'error');
                button.disabled = false;
...
function endOfWeekFromMonday(value) {
...
async function renderNewReport() {
    ...
    try {
      ...
      <form id="new-report-form" class="card form-card premium-form">
        ...
        wireGlobalControls();
        const form = document.querySelector('#new-report-form');
        const clientSelect = document.querySelector('#report-client');
        const metricsFields = document.querySelector('#report-metrics-fields');
        ...
        startInput?.addEventListener('change', () => { if (endInput && startInput.value)
            endInput.value = endOfWeekFromMonday(startInput.value); });
        renderMetricFields();
        ...
        form?.addEventListener('submit', async (event) => {
            ...
            if (!client) {
                document.querySelector('#error-clientCode').textContent = 'Selecione um cliente.';
                return;
            }
            const metrics = client.theses.map((thesis) => ({
            ...
            const payload = { clientCode: client.code, periodStart: String(fd.get('periodStart') ?? ''), periodEnd: String(fd.get('periodEnd') ?? ''), metrics };
            const button = form.querySelector('button[type="submit"]');
            if (button)
                button.disabled = true;
            try {
                ...
                for (const [field, message] of Object.entries(typed.fieldErrors ?? {})) {
                    ...
                    const el = document.querySelector(`#error-${simple}`);
                    if (el)
                        el.textContent = message;
                }
                toast(typed.message, 'error');
...
async function renderAdminReport(id) {
    loadingAdmin('reports');
    try {
      ...
      <aside class="report-detail-side"><article class="card detail-meta-card"><span class="eyebrow">DETALHES</span><div class="detail-meta-row"><span>Status</span>${statusBadge(report)}</div><div class="detail-meta-row"><span>Período</span><strong>${e(period(report))}</strong></div><div class="detail-meta-row"><span>Criado em</span><strong>${e(dateTime(report.sentAt))}</strong></div><div class="detail-meta-row"><span>Respondido em</span><strong>${e(dateTime(report.respondedAt))}</strong></div><div class="detail-meta-row"><span>Teses</span><strong>${report.theses.length}</strong></div></article></aside></div>
        ...
        wireGlobalControls();
        wireCopyButtons();
        document.querySelector('#export-report')?.addEventListener('click', async (event) => {
            const button = event.currentTarget;
            button.disabled = true;
            try {
                await downloadXlsx(`/api/exports/report/${report.id}`, {});
                toast('Relatório exportado em XLSX.', 'success');
            }
            catch (error) {
                toast(error instanceof Error ? error.message : 'Não foi possível exportar.', 'error');
            }
...
function createEmptyDraft(metrics) {
...
function validateClientDraft(draft, leads) {
    const errors = {};
    if (!draft.attendance)
        errors.attendance = 'Selecione uma opção.';
    if (!draft.quality)
        errors.quality = 'Selecione uma opção.';
    if (!draft.contractStatus)
        errors.contractStatus = 'Selecione uma opção.';
    if (!draft.problems.length)
        errors.problems = 'Selecione pelo menos uma opção.';
    if (draft.attendance !== 'ALL' && draft.attendanceCount !== null && (!Number.isInteger(draft.attendanceCount) || draft.attendanceCount < 0 || draft.attendanceCount > leads))
        errors.attendanceCount = `Informe um número entre 0 e ${leads}.`;
    if (draft.contractStatus === 'YES' && (draft.contractCount === null || !Number.isInteger(draft.contractCount) || draft.contractCount < 1 || draft.contractCount > leads))
        errors.contractCount = `Informe um número entre 1 e ${leads}.`;
    if (draft.contractStatus === 'NEGOTIATING' && (draft.negotiationCount === null || !Number.isInteger(draft.negotiationCount) || draft.negotiationCount < 0 || draft.negotiationCount > leads))
        errors.negotiationCount = `Informe um número entre 0 e ${leads}.`;
    if (draft.problems.includes('NONE') && draft.problems.length > 1)
        errors.problems = '“Nenhum problema relevante” deve ficar sozinho.';
    return errors;
}
...
function checkButton(value, selected) {
...
function publicMetricCards(metrics) {
...
function thesisForm(metrics, draft, index, errors) {
...
function reviewCard(metrics, draft, index) {
...
function safeDraftFromSession(token, report) {
    try {
        const raw = sessionStorage.getItem(`mn-insights:${token}`);
        if (!raw)
            return report.metrics.map(createEmptyDraft);
        const parsed = JSON.parse(raw);
        if (!Array.isArray(parsed.drafts) || parsed.drafts.length !== report.metrics.length)
            return report.metrics.map(createEmptyDraft);
...
async function renderPublicReport(token) {
    ...
    applyTheme(currentTheme());
    try {
        const data = await api(`/api/public/reports/${encodeURIComponent(token)}`);
        const report = data.report;
        if (data.blocked) {
        ...
        const collectTextFields = () => {
            ...
            if (draft.contractStatus === 'YES') {
                const contractCount = document.querySelector('#contract-count');
                draft.contractCount = contractCount?.value ? Number(contractCount.value) : null;
            }
            else
                draft.contractCount = null;
            if (draft.contractStatus === 'NEGOTIATING') {
                const negotiationCount = document.querySelector('#negotiation-count');
                draft.negotiationCount = negotiationCount?.value ? Number(negotiationCount.value) : negotiationCount?.value === '0' ? 0 : null;
            }
            else
                draft.negotiationCount = null;
            draft.otherProblem = document.querySelector('#other-problem')?.value ?? draft.otherProblem;
            draft.campaignObservation = document.querySelector('#campaign-observation')?.value ?? draft.campaignObservation;
            draft.agencyFeedback = document.querySelector('#agency-feedback')?.value ?? draft.agencyFeedback;
            persist();
        };
        const render = () => {
        ...
        const focusFirstError = () => {
            const firstBlock = document.querySelector('[data-error-field] .field-error');
            firstBlock?.scrollIntoView({ behavior: 'smooth', block: 'center' });
            firstBlock?.closest('[data-error-field]')?.querySelector('button, input, textarea')?.focus();
        };
        const wirePublicEvents = () => {
            ...
            if (reviewing) {
                ...
                document.querySelectorAll('.edit-thesis').forEach((button) => button.addEventListener('click', () => { reviewing = false; step = Number(button.dataset.index ?? 0); errors = {}; render(); }));
                document.querySelector('#confirm-submit')?.addEventListener('click', async () => {
                    ...
                    try {
                        ...
                        await api(`/api/public/reports/${encodeURIComponent(token)}/submit`, { method: 'POST', body: JSON.stringify(payload) });
                        sessionStorage.removeItem(`mn-insights:${token}`);
                        submitted = true;
                        render();
                    }
            ...
            document.querySelectorAll('.option-btn').forEach((button) => button.addEventListener('click', () => {
                ...
                if (group === 'attendance') {
                    draft.attendance = value;
                    if (draft.attendance === 'ALL')
                        draft.attendanceCount = null;
                }
                if (group === 'quality')
                    draft.quality = value;
                if (group === 'contractStatus') {
                    draft.contractStatus = value;
                    if (draft.contractStatus !== 'YES')
                        draft.contractCount = null;
                    if (draft.contractStatus !== 'NEGOTIATING')
                        draft.negotiationCount = null;
            ...
            document.querySelectorAll('.check-btn').forEach((button) => button.addEventListener('click', () => {
                collectTextFields();
                const draft = drafts[step];
            ...
            document.querySelector('#next-step')?.addEventListener('click', () => {
                ...
                if (step < report.metrics.length - 1) {
                    step += 1;
                    errors = {};
                }
                else
            ...
            document.querySelector('#negotiation-count')?.addEventListener('keydown', (event) => {
...
function wireAuthCharacter() {
    ...
    const stateFromFocus = () => {
        ...
        if (document.activeElement === password || document.activeElement === confirm) {
            setState('password');
            return;
        }
        setState('neutral');
    ...
    for (const field of [email, password, confirm])
        field?.addEventListener('blur', () => window.setTimeout(stateFromFocus, 0));
    ...
    if (character && stage && finePointer && !reducedMotion) {
        ...
        window.addEventListener('pointermove', (event) => {
            ...
            if (!frame)
                frame = window.requestAnimationFrame(updateEyes);
        }, { passive: true });
...
async function renderAuthPage(mode) {
    ...
    form?.addEventListener('submit', async (event) => {
        event.preventDefault();
        document.querySelectorAll('#auth-form .field-error').forEach((el) => { el.textContent = ''; });
        const fd = new FormData(form);
        const email = String(fd.get('email') ?? '').trim();
        const password = String(fd.get('password') ?? '');
        const errorEl = document.querySelector('#auth-error');
        if (!email) {
            const target = document.querySelector('#error-email');
            if (target)
                target.textContent = 'Informe seu e-mail.';
            character.setState('error');
            return;
        }
        if (setup && password !== String(fd.get('confirm') ?? '')) {
            if (errorEl)
        ...
        if (button) {
            button.disabled = true;
            button.textContent = setup ? 'Criando acesso…' : 'Entrando…';
        }
        try {
            const payload = setup ? { name: String(fd.get('name') ?? ''), email, password } : { email, password };
            const state = await api(setup ? '/api/auth/setup' : '/api/auth/login', { method: 'POST', body: JSON.stringify(payload) });
            adminCsrfToken = state.csrfToken ?? '';
            adminUserName = state.user?.name ?? 'Equipe MN';
            ...
            const typed = error;
            if (errorEl)
                errorEl.textContent = typed.message;
            for (const [field, message] of Object.entries(typed.fieldErrors ?? {})) {
            ...
            if (button) {
                button.disabled = false;
                button.innerHTML = `${setup ? 'Criar acesso interno' : 'Entrar'} ${icon('arrow', 17)}`;
            }
        }
    });
}
function renderError(error, admin, active) {
...
async function route() {
    ...
    if (path === '/setup-admin')
        return renderAuthPage('setup');
    const publicReport = path.match(/^\/r\/([A-Za-z0-9_-]{16,80})$/);
    if (publicReport)
        return renderPublicReport(publicReport[1]);
    ...
    if (client)
        return renderClient(client[1]);
    if (path === '/relatorios')
        return renderReports();
    if (path === '/teses')
        return renderTheses();
    if (path === '/relatorios/novo')
        return renderNewReport();
    const adminReport = path.match(/^\/relatorios\/(\d+)$/);
    if (adminReport)
        return renderAdminReport(Number(adminReport[1]));
