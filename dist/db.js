...
import { CONTRACT_STATUS_VALUES, LEAD_ATTENDANCE_VALUES, LEAD_PROBLEM_VALUES, LEAD_QUALITY_VALUES, REPORT_STATUSES, THESIS_SLUGS } from './shared/types.js';
...
function cleanOptionalText(value, maxLength = 700) {
...
function assertPositiveOptionalInteger(value, fieldName, max) {
...
function validateDraft(draft, leads) {
    const errors = {};
    if (!inList(draft.attendance, LEAD_ATTENDANCE_VALUES))
        errors.attendance = 'Selecione quantos leads foram atendidos.';
    if (!inList(draft.quality, LEAD_QUALITY_VALUES))
        errors.quality = 'Selecione uma avaliação de qualidade.';
    if (!inList(draft.contractStatus, CONTRACT_STATUS_VALUES))
        errors.contractStatus = 'Selecione a situação dos contratos.';
    ...
    const problems = Array.isArray(draft.problems)
        ? Array.from(new Set(draft.problems.filter((item) => inList(item, LEAD_PROBLEM_VALUES))))
        : [];
    ...
    if (draft.contractStatus === 'YES') {
        if (draft.contractCount === null || draft.contractCount === undefined) {
            ...
            try {
                ...
                if (contractCount !== null && contractCount < 1)
                    errors.contractCount = 'Informe pelo menos 1 contrato.';
            }
            catch (error) {
                if (error instanceof ValidationError)
                    Object.assign(errors, error.fieldErrors);
    ...
    if (Object.keys(errors).length > 0)
        throw new ValidationError('Revise os campos destacados.', errors);
    return {
...
function migrate(db) {
  ...
  `);
    const feedbackColumns = new Set(rows(db.prepare('PRAGMA table_info(feedbacks)').all()).map((item) => item.name));
    ...
    const adminColumns = new Set(rows(db.prepare('PRAGMA table_info(admin_users)').all()).map((item) => item.name));
  ...
  `);
    ...
    try {
        ...
        for (const item of AGENCY_BASE_CLIENTS) {
            const found = row(existingClient.get(item.code));
            let clientId;
            if (found) {
                clientId = found.id;
            ...
            item.thesisSlugs.forEach((slug, index) => {
                const thesis = row(thesisBySlug.get(slug));
                if (!thesis)
                    throw new Error(`Tese da base da agência não cadastrada: ${slug}`);
...
function listTheses(db) {
    return rows(db.prepare('SELECT id, slug, name FROM theses ORDER BY id').all()).map((item) => ({
        id: item.id,
        slug: item.slug,
        name: item.name
    }));
}
function clientTheses(db, clientId) {
...
function metricsForReport(db, reportId) {
    return rows(db.prepare(`
...
function feedbacksForReport(db, reportId) {
    const feedbackRows = rows(db.prepare(`
...
function insertNotificationOnce(db, input) {
    db.prepare(`
...
export function syncReportNotifications(db, now = new Date()) {
    const today = saoPauloDate(now);
    const awaiting = rows(db.prepare(`
...
function notificationFromRow(item) {
    return {
        id: item.id,
        type: item.type,
        title: item.title,
...
export function listNotifications(db, limit = 30, now = new Date()) {
  ...
  `).all(safeLimit)).map(notificationFromRow);
    const unread = row(db.prepare('SELECT COUNT(*) AS count FROM notifications WHERE read_at IS NULL').get());
    return { notifications: items, unreadCount: Number(unread?.count ?? 0) };
}
export function markNotificationRead(db, notificationId, now = new Date()) {
...
export function listClients(db) {
    const clientRows = rows(db.prepare('SELECT id, code, name, active, created_at FROM clients ORDER BY code').all());
  ...
  `);
    return clientRows.map((item) => {
        const latestRow = row(latestStmt.get(item.id));
        const base = {
            id: item.id,
            code: item.code,
            name: item.name,
        ...
        if (latestRow)
            base.latestReport = mapReport(db, latestRow);
        return base;
    });
}
export function getClientDetail(db, code) {
    const clientRow = row(db.prepare('SELECT id, code, name, active, created_at FROM clients WHERE code = ?').get(code));
    if (!clientRow)
        return null;
    const client = {
        id: clientRow.id,
    ...
    if (reports[0])
        client.latestReport = reports[0];
    return { client, reports };
}
export function getReportById(db, reportId) {
    const reportRow = row(db.prepare(`
    SELECT r.id, r.client_id, c.code AS client_code, c.name AS client_name,
           r.period_start, r.period_end, r.status, r.token, r.sent_at, r.responded_at
...
export function submitReportFeedback(db, token, payload) {
    const report = getReportByToken(db, token);
    if (!report)
        throw new ValidationError('Relatório não encontrado.');
    ...
    if (payload.answers.length !== report.metrics.length)
        throw new ValidationError('Responda todas as teses antes de confirmar.');
    ...
    const validated = payload.answers.map((answer) => {
        ...
        if (!metrics)
            throw new ValidationError('Tese inválida.');
        return validateDraft(answer, metrics.leads);
    });
    db.exec('BEGIN IMMEDIATE;');
    try {
        const fresh = row(db.prepare('SELECT status FROM reports WHERE id = ?').get(report.id));
        if (!fresh || fresh.status === 'RESPONDED')
            throw new ConflictError('Este relatório já foi respondido.');
    ...
    `);
        ...
        for (const answer of validated) {
            const result = insertFeedback.run(report.id, answer.thesisId, answer.attendance, answer.attendanceCount, answer.quality, answer.contractStatus, answer.contractCount, answer.negotiationCount, cleanOptionalText(answer.otherProblem, 300), cleanOptionalText(answer.campaignObservation, 700), cleanOptionalText(answer.agencyFeedback, 700));
            const feedbackId = Number(result.lastInsertRowid);
            for (const problem of answer.problems)
                insertProblem.run(feedbackId, problem);
        }
    ...
    if (!updated)
        throw new Error('Falha ao carregar o relatório atualizado.');
    return updated;
}
export function createClient(db, payload) {
    const name = typeof payload?.name === 'string' ? payload.name.trim() : '';
    const thesisSlugs = Array.isArray(payload?.thesisSlugs)
        ? Array.from(new Set(payload.thesisSlugs.filter((slug) => inList(slug, THESIS_SLUGS))))
        : [];
    const errors = {};
    if (name.length < 3 || name.length > 100)
        errors.name = 'Informe um nome entre 3 e 100 caracteres.';
    if (thesisSlugs.length < 1 || thesisSlugs.length > 2)
        errors.thesisSlugs = 'Selecione uma ou duas teses.';
    if (Object.keys(errors).length > 0)
        throw new ValidationError('Revise os dados do cliente.', errors);
    const maxRow = row(db.prepare("SELECT MAX(CAST(SUBSTR(code, 3) AS INTEGER)) AS max_num FROM clients WHERE code GLOB 'MN[0-9][0-9][0-9]*'").get());
    ...
    try {
        ...
        thesisSlugs.forEach((slug, index) => {
            const thesis = row(thesisStmt.get(slug));
            if (!thesis)
                throw new ValidationError('Tese não cadastrada.');
    ...
    if (!detail)
        throw new Error('Falha ao carregar o cliente criado.');
...
function validateReportPeriod(startValue, endValue) {
    const start = parseIsoDate(startValue);
    const end = parseIsoDate(endValue);
    const errors = {};
    if (!start)
        errors.periodStart = 'Informe a segunda-feira de início do período.';
    if (!end)
        errors.periodEnd = 'Informe o domingo de encerramento do período.';
    if (start && start.getUTCDay() !== 1)
        errors.periodStart = 'O período deve começar em uma segunda-feira.';
    if (end && end.getUTCDay() !== 0)
        errors.periodEnd = 'O período deve terminar em um domingo.';
    if (start && end) {
        const days = Math.round((end.getTime() - start.getTime()) / 86_400_000);
        if (days !== 6)
            errors.periodEnd = 'O relatório deve representar exatamente uma semana, de segunda a domingo.';
    }
    if (Object.keys(errors).length)
        throw new ValidationError('Revise o período do relatório.', errors);
...
function parseNonNegativeInteger(value, field, max = 10_000_000) {
    if (typeof value !== 'number' || !Number.isInteger(value) || value < 0 || value > max) {
...
export function createReport(db, payload) {
    ...
    if (!client)
        throw new ValidationError('Cliente não encontrado.', { clientCode: 'Selecione um cliente válido.' });
    ...
    `);
        for (const metric of normalized) {
            const thesis = row(thesisStmt.get(metric.slug));
            if (!thesis)
                throw new ValidationError('Tese não cadastrada.');
        ...
        if (!created)
            throw new Error('Falha ao carregar o relatório criado.');
        return created;
    }
    catch (error) {
        db.exec('ROLLBACK;');
...
export function getDashboard(db, filterInput = {}) {
    ...
    const filteredReports = thesisSlug
        ...
        : reports;
    const latestPeriodRow = row(db.prepare('SELECT period_start, period_end FROM reports ORDER BY period_start DESC LIMIT 1').get());
...
export function recordAudit(db, input) {
...
export function recordExport(db, input) {
...
export function listExports(db, limit = 30) {
