...
export function hasAdminUser(db) {
    const result = db.prepare("SELECT COUNT(*) AS count FROM admin_users WHERE role = 'INTERNAL'").get();
    return Number(result?.count ?? 0) > 0;
}
export function createInitialAdmin(db, input) {
    if (hasAdminUser(db))
        throw new ValidationError('O usuário interno inicial já foi configurado.');
    const name = typeof input.name === 'string' ? input.name.trim().slice(0, 80) : '';
    if (name.length < 2)
        throw new ValidationError('Informe o nome do usuário interno.', { name: 'Informe pelo menos 2 caracteres.' });
...
export function authenticateAdmin(db, emailInput, passwordInput) {
    if (typeof passwordInput !== 'string' || passwordInput.length > 160)
        return null;
    let email;
    try {
  ...
  `).get();
    if (!user || (user.email !== null && user.email !== email))
        return null;
    ...
    if (expectedBytes.byteLength !== actualBytes.byteLength || !timingSafeEqual(expectedBytes, actualBytes))
        return null;
    if (user.email === null)
        db.prepare('UPDATE admin_users SET email = ? WHERE id = ? AND email IS NULL').run(email, user.id);
    return { id: user.id, name: user.name, role: user.role };
}
export function createAdminSession(db, user) {
