...
function inlineCell(ref: string, cell: Cell): string {
...
function worksheetXml(spec: SheetSpec): string {
...
function zip(files: Array<{ name: string; content: string | Uint8Array }>): Uint8Array {
...
function reportStatusLabel(report: ReportSummary): string {
  if (report.status === 'RESPONDED') return 'Respondido';
  return todaySaoPaulo() > report.periodEnd ? 'Atrasado' : 'Aguardando';
}

const attendance: Record<string, string> = { FEW: 'Poucos', HALF: 'Cerca da metade', MOST: 'A maioria', ALL: 'Todos' };
const quality: Record<string, string> = { POOR: 'Ruim', REGULAR: 'Regular', GOOD: 'Boa', VERY_GOOD: 'Muito boa' };
const contract: Record<string, string> = { YES: 'Sim', NO: 'Não', NEGOTIATING: 'Ainda estão em negociação' };
const problem: Record<string, string> = { NO_PROFILE: 'Não tinham direito/perfil', NO_RESPONSE: 'Não responderam', NO_INTEREST: 'Sem interesse', HAS_LAWYER: 'Já tinham advogado', OUTSIDE_REGION: 'Fora da região/público', NONE: 'Nenhum problema relevante', OTHER: 'Outro' };
...
function buildSheets(reports: ReportSummary[]): SheetSpec[] {
  ...
  const summaryData = reports.map((report) => {
    ...
    return [
      ...
      { value: investment, style: 5, type: 'number' as const }, { value: leads, style: 4, type: 'number' as const },
      { value: contracts, style: 4, type: 'number' as const }, { value: formatDateTime(report.respondedAt), style: 4 }
    ];
  });
  const summaryRows: Cell[][] = [...titleRows('Resumo da exportação', scopeTitle, summaryHeaders.length), summaryHeaders.map((value) => ({ value, style: 3 })), ...summaryData];
  ...
  const metricRows = reports.flatMap((report) => report.metrics.map((metric) => {
    ...
    return [
      ...
      { value: metric.thesisName, style: 4 }, { value: metric.investmentCents / 100, style: 5, type: 'number' as const },
      { value: metric.leads, style: 4, type: 'number' as const }, { value: metric.cplCents / 100, style: 5, type: 'number' as const },
      { value: metric.cpcCents / 100, style: 5, type: 'number' as const }, { value: contracts, style: 4, type: 'number' as const },
      costPerContract === null ? { value: '', style: 4 } : { value: costPerContract, style: 5, type: 'number' as const }
    ];
  }));
  const metrics: Cell[][] = [...titleRows('Métricas por tese', scopeTitle, metricHeaders.length), metricHeaders.map((value) => ({ value, style: 3 })), ...metricRows];
  ...
  const answerRows = reports.flatMap((report) => report.feedbacks.map((feedback) => [
    ...
    { value: feedback.attendanceCount ?? '', style: 4, ...(feedback.attendanceCount === null ? {} : { type: 'number' as const }) },
    ...
    { value: feedback.contractCount ?? '', style: 4, ...(feedback.contractCount === null ? {} : { type: 'number' as const }) },
    { value: feedback.negotiationCount ?? '', style: 4, ...(feedback.negotiationCount === null ? {} : { type: 'number' as const }) },
  ...
  const answers: Cell[][] = [...titleRows('Respostas dos clientes', scopeTitle, answerHeaders.length), answerHeaders.map((value) => ({ value, style: 3 })), ...answerRows];
...
export function buildReportsXlsx(reports: ReportSummary[]): Uint8Array {
...
export function exportFilename(reports: ReportSummary[]): string {
  const clean = (value: string) => value.normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^A-Za-z0-9_-]+/g, '_').replace(/^_+|_+$/g, '');
  if (reports.length === 1) {
